...
class ControlRow extends Component {
	...
	makePick(picked, newState) {
		...
		toggleValues = _.mapValues(
			toggleValues,
			(value, key) => newState && key == picked // eslink-disable-line
		);
		
		//if newState is false, we want to reset
		// only resetting filters if newState is false since that means a toggle was unclicked without a new one being selected.
